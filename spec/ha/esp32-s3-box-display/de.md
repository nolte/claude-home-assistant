...
- Grafikdesign-Beratung (Farbenlehre, Ikonografie, typografische Ästhetik) — diese Spec regelt die Mechanik der Platzierung, nicht den darauf angewandten Geschmack; das auf dieser Mechanik aufsetzende Design-System ist [`ha/esp32-s3-box-display-design`](../esp32-s3-box-display-design/de.md)
...
- Ein neues Panel oder eine neue Geometrie **SOLLTE [SHOULD]** mit `show_test_card: true` verifiziert werden, bevor ein Layout debuggt wird, damit eine falsche `color_order` oder ein falscher Offset zuerst ausgeschlossen ist — dabei **MUSS [MUST]** jedoch beachtet werden, dass dies dem Refresh-Modell weiter unten widerspricht: Die Dokumentation hält fest, dass die Testkarte bei gesetztem `update_interval: never` nicht gezeichnet wird, was diese Spec ansonsten vorschreibt. Die Karte anzuzeigen heißt daher, vorübergehend ein echtes `update_interval` wiederherzustellen und danach `never` zurückzusetzen `[doc]`
...
- Eine **Hintergrundfarbe** **MUSS [MUST]** übergeben werden, wenn der Font antialiast ist (`bpp` größer als 1), und sie **MUSS [MUST]** an der Position stehen, die das jeweilige Overload erwartet — beide unterscheiden sich, das ist die Falle:
  ...
  - `it.printf(x, y, font, foreground, background, align, format, …)` — Hintergrund an **fünfter** Stelle, vor dem Align, weil die variadischen Format-Argumente zuletzt stehen müssen. Die Dokumentation nennt es „the full version of `printf`" und zeigt es genau zu diesem Zweck; hier war es zuvor als undokumentiert vermerkt, was nicht mehr zutrifft.

  Einen Hintergrund in der `print`-Position an `printf` zu übergeben scheitert nicht laut — es bindet ein anderes Overload, und die Farbe wird als Format-Argument verbraucht `[doc]`
...
- Jedes Bild **MUSS [MUST]** unter `image:` mit einer `id:` und einem Compile-Zeit-`resize:` für die einzunehmende Box deklariert werden, und die Deklarationsform **MUSS [MUST]** gegen die `min_version` der eigenen Config gewählt werden, weil sich beide Formen zeitlich nicht überlappen:
  - **`min_version: 2026.7.0` oder neuer** — die Plattform-Form nutzen (`platform: file` für zur Compile-Zeit eingebettete Assets, `animation` für mehrbildrige, `online_image` für Laufzeit-Download). `CONF_PLATFORM` taucht in der `image`-Komponente erstmals bei **2026.7.0** auf; bei 2026.6.0 und früher fehlt es, unterhalb dieses Floors erzeugt es also eine Config, die nicht validiert.
  - **unterhalb 2026.7.0** — die ältere plattformlose Form nutzen, wie alle drei Referenzkonfigurationen es an ihren gepinnten Floors noch tun.

  Die plattformlose Form ist ein Migrations-Shim, keine unterstützte Alternative: Die Komponente führt `LEGACY_REMOVAL_VERSION = "2027.1.0"` und schreibt Alt-Einträge transparent nach `platform: file` um — sie verschwindet also in **2027.1.0**. Jede Config auf der Altform muss bis dahin umgezogen sein. Zu beachten außerdem: `resize` passt das Bild **unter Wahrung des Seitenverhältnisses in die Box ein** — `resize: 320x240` füllt dieses Panel nur bei einer 4:3-Quelle `[doc]` `[src]`
...
- `transparency: chroma_key` oder `alpha_channel` **KANN [MAY]** genutzt werden, um ein Bild über einen Hintergrund zu komponieren, statt den Hintergrund in jedes Asset einzubacken. Die Dokumentation hält fest, dass die Bildfunktionen des Display-Lambdas „will draw or not draw the pixel, no blending with the background will be done", und dass Alpha-Blending „useful mainly when using LVGL" sei — das gilt jedoch nur für `RGB565` und `RGB`, deren Zeichenpfade binär auf `alpha >= 0x80` entscheiden. Für **`GRAYSCALE` ist es falsch**: Dieser Pfad berechnet `on = gray/255` und blendet `color_on` pro Pixel gegen `color_off`, ein `GRAYSCALE`-`alpha_channel`-Bild *wird* im Immediate-Mode also alpha-geblendet, bei einem Byte pro Pixel. Dieses Verhalten ist die Grundlage des Icon-Systems in [`ha/esp32-s3-box-display-design`](../esp32-s3-box-display-design/de.md) und ist aus `esphome/components/image/image.cpp` belegt, nicht aus der Dokumentation `[doc]` `[src]`
...
- `draw_rounding` **SOLLTE [SHOULD]** auf dem Default 2 bleiben, solange keine Artefakte auftreten; es rundet Zeichenbereiche auf eine Grenze für Controller, die das verlangen, und die `mipi_spi`-Dokumentation hält ausdrücklich fest, dass der Wert **„Must be a power of 2"** ist — ein erhöhter Wert wird also aus 4, 8, 16 gewählt, nicht frei `[doc]`
...
- [ ] Jeder Font deklariert eine explizite `size` und einen begrenzten Zeichensatz; jedes Bild deklariert ein explizites `resize` und `type`, nutzt die zur `min_version` der Config passende Deklarationsform (Plattform-Form ab `2026.7.0`, sonst Altform) und keinen nicht existierenden Typ `RGBA`

...
- **MUST** declare each image under `image:` with an explicit **`platform:`** (`file` for assets embedded at compile time, `animation` for multi-frame, `online_image` for runtime download), an `id:`, and a compile-time `resize:` for the box it will occupy. `image:` became a platform component upstream; the reference configs still use the older platform-less form, which is accepted but is not what the documentation shows. Note that `resize` **fits inside** the given box preserving aspect ratio — `resize: 320x240` fills this panel only for a 4:3 source `[doc]` `[ref-config]`
...
- [ ] Every font declares an explicit `size` and a bounded glyph set; every image declares an explicit `platform`, `resize`, and `type`, and no image uses the non-existent `RGBA` type

...
- Deployment in eine laufende HA-Instanz oder Laufzeit-Verifikation als Teil des Standard-Generierungslaufs — das läuft über die `ha-integration-deployer`- / `ha-integration-verifier`-Agenten und nur innerhalb der Opt-in-`deploy_ready`-Lifecycle-Phase hinter einem zweiten Gate (außerhalb des Generierungs-only-Scopes)
...
- **MUSS [MUST]** deklarative Read-Type-Entities (Datapoint-/Schema-getrieben) auf `ha-entity-description-map` abbilden und aktive, befehlsgetriebene Plattformen (climate/cover/light/fan/lock/media_player/…) auf `ha-entity-platform-add`
...
- **KANN [MAY]**, nur wenn `deploy_ready` gesetzt ist und nur nach einem **zweiten expliziten menschlichen Gate**, das vom Plan-Gate verschieden ist, die Deploy→Verify→Review-Lifecycle-Phase ausführen: `ha-dev-instance-provisioner` (nur falls kein Dev-HA-Pod existiert) → `ha-integration-deployer` → `ha-integration-verifier` → `ha-integration-reviewer` (der Release-/Pre-PR-Gesamtblick-Durchlauf, legitim zu diesem separaten Post-Gate-Zeitpunkt ausgeführt, nie zusätzlich zum In-Flow-Gate). Der Generierungs-only-Default und sein einzelnes Gate bleiben erhalten, wenn `deploy_ready` nicht gesetzt ist
- **MUSS [MUST]** in dieser Phase bei einem `ha-integration-verifier`-FAIL oder einem `ha-integration-reviewer`-NEEDS-WORK jedes Finding an den zuständigen Authoring-/Remediation-Skill zurückrouten (das Audit-Defizit-Muster), den Fix neu dispatchen und dann neu deployen und neu verifizieren — ein geschlossener Feedback-Loop, kein terminaler beschreibender Bericht; nie `kubectl delete pod` (ein Code-Refresh ist `kill 1`, im Besitz des `ha-integration-deployer`-Agenten)
...
- **MUSS [MUST]** im Generierungs-only-Default (`deploy_ready` nicht gesetzt) auf die Operator-Folge-Schritte hinweisen (ein gebündelter Gesamtblick-Durchlauf über den `ha-integration-reviewer`-Agenten als **Release-/Pre-PR**-Review, Deploy/Verify über die `ha-integration-deployer`- und `ha-integration-verifier`-Agenten), ohne sie auszuführen — der `ha-integration-reviewer`-Folge-Schritt ergänzt Cross-Cutting + Drift *zusätzlich zu* Quality-Scale + Security und **MUSS NICHT [MUST NOT]** das In-Flow-Quality+Security-Gate dieses Laufs erneut fahren. Ist `deploy_ready` gesetzt, werden diese Folge-Schritte stattdessen von der Opt-in-Lifecycle-Phase hinter dem zweiten Gate ausgeführt, und der Bericht enthält das Deploy/Verify/Review-Ergebnis und jeden Fix-Zyklus
...
- [ ] In dieser Phase routet ein `ha-integration-verifier`-FAIL oder `ha-integration-reviewer`-NEEDS-WORK zurück an den zuständigen Skill und treibt Deploy→Verify erneut, statt mit einem beschreibenden Bericht zu terminieren
- [ ] Der Generierungs-only-Default (und sein einzelnes Gate) bleibt erhalten, wenn `deploy_ready` nicht gesetzt ist

## Offene Fragen

- **Review-Agent vs. Review-Skills (entschieden)**: Der gebündelte `ha-integration-reviewer`-Agent (ein Aufruf, der Quality-Scale, Security und übergreifende Konsistenz read-only zusammenfasst) existiert bereits, ist aber per Contract eine *fire-and-forget* Operator-Folge-Prüfung (pre-PR/pre-release), die nie von einem Orchestrator dispatcht wird und die Einzel-Audit-Skills nie ersetzt. **Entscheidung (Variante b):** Die zwei Audit-Skills sind das **In-Flow-Acceptance-Gate** (interaktiv, sichtbar, einmal im Lauf ausgeführt); `ha-integration-reviewer` ist der separate **Release-/Pre-PR-Gesamtblick-Durchlauf**. Die Trennung ist **zeitlich, nicht additiv** — ein Lauf, der das In-Flow-Gate besteht, fährt nicht zusätzlich den gebündelten Agenten für dieselben zwei Dimensionen, sodass die Nutzerin die Quality+Security-Bewertung nie doppelt bezahlt. Siehe die Dispatch-/Plan-Regeln und Validierung & Bericht.

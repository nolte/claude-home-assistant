...
description: "Scaffolds one complete ESPHome device-config YAML file — substitutions-driven naming, shared blocks via packages, secrets-referenced wifi with AP fallback, encrypted native api, ota, logger, and the first sensor/actuator platform blocks — conforming to spec/ha/esphome-config-patterns. Gathers device name, board family, shared-concern set, and initial platforms, runs a pre-flight (collision, secrets example, device-config root), writes the new <device-name>.yaml, and reports which secrets keys the operator must fill. Activate on \"scaffold an ESPHome device config\", \"new ESPHome device YAML\", \"add a new ESPHome node\", or equivalent German requests. Do not activate for extending an existing device file (ha-esphome-config-augment), an ESPHome custom component in C++/Python (out of scope; no component axis yet), an HA Custom Integration (ha-integration-scaffold), or compiling/flashing/deploying to a device or HA instance."
tags: [home-assistant, esphome, scaffolding, yaml]
phase: build
summary: "Scaffolds one spec-conformant ESPHome device-config YAML (packages, secrets, encrypted api, ota) for a new device."
summary_de: "Scaffoldet eine spec-konforme ESPHome-Device-Config-YAML (packages, secrets, verschlüsselte api, ota) für ein neues Gerät."
use_when:
...
3. **No literal secrets, ever.** wifi, api encryption, and ota reference `!secret` keys; report the keys the operator must add to `secrets.yaml` (and extend `secrets.yaml.example` when present).
...
3. Inventory shared packages/snippets and `secrets.yaml.example`; note which referenced secret keys already exist.
...
4. Report: written path, secrets keys to fill, validation status, and the augment skill as the follow-up entry point.

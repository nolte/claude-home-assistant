...
description: >-
  ...
  (`ha-integration-deployer`), don't use to restart HA, don't use to test
...
This agent operationalises the verify choreography defined in `spec/ha/dev-environment/en.md`. It is the read-only sibling of `ha-integration-deployer`: where the deploy agent is the only path that legally writes to the pod, this agent is the only path you should use to read the pod's state without accidentally deploying.

...
- **Claude-Code-Plugin-Marketplace (Distributionsinfrastruktur)** — _Kategorie_: operator · _Oberfläche_: `.claude-plugin/marketplace.json`, Install-Pfad über den Plugin-Marketplace-Mechanismus · _erwartet_: Plugin-Manifest ist marketplace-valid; Versionsbumps in `plugin.json` + `marketplace.json` synchron; Skills/Agents bleiben rückwärtskompatibel innerhalb einer Major-Version · _Status_: `assumed` · _Kritikalität_: peripheral (heute, solange das Plugin nicht öffentlich gelistet ist)

...
Spec: `spec/claude/ha-repairs-add/en.md` (EN canonical) / `spec/claude/ha-repairs-add/de.md` (DE translation).
...
2. **Read `spec/ha/repairs/en.md` first.** Do not generate from memory.
...
6. **No hard-coded user strings.** Every `translation_key` lives in `strings.json` under `issues:` with `title`/`description`, all `translation_placeholders` resolved (see `spec/ha/translations/en.md`) — placeholder values must be strings (`str(...)` any non-string value).

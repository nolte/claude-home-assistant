...
- **SOLLTE NICHT [SHOULD NOT]** `threshold` auf ein **stark verrauschtes** Rohsignal ohne Glättung **und** ohne `hysteresis` anwenden — der Sensor flattert dann; entweder `hysteresis` setzen oder zuerst glätten und den geglätteten Wert als `entity_id` verwenden — für reine Rauschglättung `ha-automation/filter` als dediziertes Glättungswerkzeug, für eine statistische Kenngröße über die Stichproben `ha-automation/statistics` (z. B. gleitender Mittelwert)

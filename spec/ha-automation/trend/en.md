...
- **SHOULD NOT** apply `trend` to a heavily **noisy** signal without sufficient `sample_duration`/`max_samples` and without pre-smoothing — noise produces spurious trends and false `on`/`off` switches; first smooth via `ha-automation/statistics` (e.g. moving average) and feed the smoothed value as `entity_id`, or choose `min_gradient` and the sample window more conservatively

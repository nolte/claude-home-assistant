...
- **SOLLTE NICHT [SHOULD NOT]** `trend` auf ein **stark verrauschtes** Signal ohne ausreichendes `sample_duration`/`max_samples` und ohne Vorglättung anwenden — Rauschen erzeugt Schein-Trends und falsche `on`/`off`-Wechsel; zuerst glätten und den geglätteten Wert als `entity_id` verwenden — für reine Rauschglättung `ha-automation/filter` als dediziertes Glättungswerkzeug, für eine statistische Kenngröße über die Stichproben `ha-automation/statistics` (z. B. gleitenden Mittelwert) — oder `min_gradient` und das Sample-Fenster konservativer wählen

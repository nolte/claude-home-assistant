...
- **SHOULD NOT** apply `threshold` to a heavily **noisy** raw signal with neither smoothing **nor** `hysteresis` — the sensor then flaps; either set `hysteresis` or first smooth via `ha-automation/statistics` (e.g. moving average) and feed the smoothed value as `entity_id`

...
- **MUST** leave the **microSD** slot unused and satisfy local-audio needs from flash instead — the `media_player` speaker platform embeds sound files directly, which is what the reference config does for its timer chime `[ref-config]` `[policy]`

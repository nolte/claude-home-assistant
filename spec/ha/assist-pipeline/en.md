...
- **MUST** choose a speech-to-text engine against the host's actual capacity: **Speech-to-Phrase** delivers "extremely fast transcription even on a Home Assistant Green or Raspberry Pi 4" but recognises a constrained phrase set, while **Whisper** handles freer speech at a documented ~8 seconds on a Pi 4 versus under 1 second on an Intel NUC `[doc:user]`
- **SHOULD** use **Piper** as the local text-to-speech engine; it is the documented local option, "optimized for the Raspberry Pi 4" `[doc:user]`
...
- **SHOULD** prefer **on-device** detection (`microWakeWord`) for battery- or network-constrained satellites: only post-detection audio is sent, it works without network connectivity, and it ships the pre-trained models "okay nabu", "hey jarvis", and "alexa" — at the cost of smaller models with less accuracy headroom `[doc:user]`

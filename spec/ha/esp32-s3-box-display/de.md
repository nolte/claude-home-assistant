...
- Der vollständige Primitiv-Satz **MUSS [MUST]** über das `it`-Objekt innerhalb eines Lambdas angesprochen werden: `line`, `rectangle`, `filled_rectangle`, `circle`, `filled_circle`, `triangle`, `filled_triangle`, `filled_ring`, `filled_gauge`, `regular_polygon`, `filled_regular_polygon`, `draw_pixel_at` sowie `fill` und `clear` `[doc]`
...
- Jeder Font **MUSS [MUST]** als `font:`-Komponente mit explizitem `size:` in Pixeln deklariert werden; ein Font wird zur Compile-Zeit gerastert, eine nicht deklarierte Größe ist zur Laufzeit also nicht verwendbar `[doc]`
- Der Zeichensatz **MUSS [MUST]** über `glyphs:` oder `glyphsets:` bewusst eingeschränkt werden — die Referenzkonfiguration nutzt das Google-Fonts-Glyphset `GF_Latin_Core` plus eine explizite Liste erlaubter Zeichen — weil jedes zusätzliche Glyph in jeder deklarierten Größe Flash kostet `[doc]` `[ref-config]`
- An `print` / `printf` **MUSS [MUST]** eine **Hintergrundfarbe** übergeben werden, wenn der Font antialiast ist (`bpp` größer als 1): Die dokumentierte Signatur für antialiasten Text lautet `it.printf(x, y, font, foreground, background, align, format, …)`, und ein weggelassener Hintergrund führt zu falsch überblendetem Text `[doc]`
...
- Dieses Limit **SOLLTE [SHOULD]** aus der Box abgeleitet werden: Bei den 15 Pixel Figtree der Referenz in einem 280 Pixel breiten Rahmen passen rund 32 Zeichen — ein anderer Font, eine andere Größe oder Boxbreite erfordert Nachmessen, nicht Wiederverwenden der Zahl `[ref-config]` `[policy]`
...
- Jedes Bild **MUSS [MUST]** als `image:`-Komponente mit `id:` deklariert und zur Compile-Zeit über `resize:` auf die Box dimensioniert werden, die es einnehmen wird — eine vollflächige Illustration auf diesem Panel ist `resize: 320x240` `[doc]` `[ref-config]`
- `type:` **MUSS [MUST]** bewusst gewählt werden (`BINARY`, `GRAYSCALE`, `RGB565`, `RGB`, `RGBA`), weil der Typ die Kosten pro Pixel im Flash bestimmt; die Referenzkonfiguration liefert vollflächige Illustrationen als `type: RGB` mit `transparency: alpha_channel` `[doc]` `[ref-config]`
- Der Flash-Bedarf eines Bildes **SOLLTE [SHOULD]** vor dem Hinzufügen budgetiert werden: Ein 320×240-`RGB565`-Bild belegt ~150 KiB und ein `RGB`-Bild noch mehr, sodass eine Handvoll vollflächiger Bilder die Firmware dominiert `[doc]` `[policy]`
- `transparency: chroma_key` oder `alpha_channel` **KANN [MAY]** genutzt werden, um ein Bild über einen Hintergrund zu komponieren, statt den Hintergrund in jedes Asset einzubacken; Graustufenbilder mit Transparenz bleiben bei einem Byte pro Pixel `[doc]`
...
- Bild-Assets **SOLLTEN [SHOULD]** im Config-Repository unter Versionskontrolle liegen, statt darauf zu bauen, dass eine Upstream-URL zur Build-Zeit erreichbar bleibt `[policy]`
...
- Die Konstanten `Color::WHITE` / `Color::BLACK` **KÖNNEN [MAY]** genutzt werden, auf die sich die Referenzkonfiguration für Rahmen und Text stützt; zu beachten ist, dass sie in der Komponenten-Dokumentation nicht aufgeführt sind, eine Color-Komponente also die portablere Form ist `[ref-config]` `[doc]`
...
- `draw_rounding` **SOLLTE [SHOULD]** auf dem Default 2 bleiben, solange keine Artefakte auftreten; es existiert, um Darstellungsfehler bei bestimmten Controllern zu verhindern, und muss eine Zweierpotenz bleiben `[doc]`
...
- [ ] Jeder Font deklariert eine explizite `size` und einen begrenzten Zeichensatz; jedes Bild deklariert ein explizites `resize` und `type`
...
- **Portfolio-Standardweg**: Sollen BOX-Projekte in diesem Portfolio standardmäßig den Immediate-Mode-Weg wählen (deckungsgleich mit Upstream, eine reviewbare Datei) oder LVGL (touch-fähig, langfristig bessere Ergonomie) — oder bleibt die Wahl pro Projekt?
- **Wiederverwendbares Layout-Package**: Lohnt es sich, das Zonenraster und die gemeinsamen Zeichen-Scripts in ein repo-lokales ESPHome-Package auszulagern, das Device-Configs einbinden, statt sie pro Gerät neu abzuleiten?
- **Kürzungs-Heuristik**: Das 32-Zeichen-Limit ist für 15-Pixel-Figtree in einer 280-Pixel-Box gemessen. Soll das Portfolio eine kleine gemessene Tabelle pflegen (Font × Größe × Boxbreite → Zeichenbudget) oder das Budget zur Render-Zeit aus Font-Metriken berechnen (`get_baseline`, Glyphbreiten)?
- **Rotation**: Keine der Referenzkonfigurationen dreht das Panel. Wenn ein Projekt die BOX anders montiert — bekommt das Zonenraster einen gedrehten Zwilling, oder ist Rotation schlicht außerhalb des Scopes?
- **`Color::WHITE` / `Color::BLACK`**: Diese Konstanten werden von der Upstream-Referenzkonfiguration genutzt, stehen aber nicht in der Komponenten-Dokumentation. Sollen Portfolio-Configs sie zugunsten deklarierter `color:`-Komponenten meiden, oder genügt die Upstream-Nutzung als Rechtfertigung?
- **Asset-Hosting**: Die Referenzkonfiguration holt Illustrationen zur Build-Zeit von GitHub-Raw-URLs. Sollen Portfolio-Configs Bild-Assets ins Config-Repository vendoren (reproduzierbare Builds, Repo-Größe) oder Remote-Referenzen behalten (kleines Repo, Netzabhängigkeit beim Build)?
- **Animation**: ESPHome liefert neben `image:` eine `animation:`-Plattform. Gibt es auf diesem Gerät einen Anwendungsfall, der die Flash-Kosten mehrbildriger Assets rechtfertigt, wo doch der Seitenwechsel Zustandsübergänge bereits abdeckt?

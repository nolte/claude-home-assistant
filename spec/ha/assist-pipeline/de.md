...
- Die Speech-to-Text-Engine **MUSS [MUST]** gegen die tatsächliche Kapazität des Hosts **und gegen die benötigten Funktionen** gewählt werden: **Speech-to-Phrase** liefert „extremely fast transcription even on a Home Assistant Green or Raspberry Pi 4 (under one second)", unterstützt aber „only … a subset of Assist's voice commands" — die Dokumentation nennt Einkaufslisten, das **Benennen eines Timers** und Broadcasts ausdrücklich als „*not* usable out of the box". **Whisper** verarbeitet freiere Sprache mit dokumentierten ~8 Sekunden auf einem Pi 4 gegenüber unter 1 Sekunde auf einem Intel NUC. Zu beachten ist die Folge für dieses Portfolio: Ein Satellit nach [`ha/esp32-s3-box`](../esp32-s3-box/de.md) stützt sich auf die Timer-Intents — Speech-to-Phrase beschneidet also genau dessen Kernfunktion `[doc:user]` `[policy]`
- Als lokale Text-to-Speech-Engine **SOLLTE [SHOULD]** **Piper** verwendet werden; sie ist die dokumentierte lokale Option, „optimized for the Raspberry Pi 4", und erzeugt auf einem Raspberry Pi mit Modellen mittlerer Qualität „1.6s of voice in a second" — eine Durchsatzangabe, die man gegen die Länge der tatsächlich gesprochenen Antworten halten sollte `[doc:user]`
...
- Die **Supervisor-Anforderung** **MUSS [MUST]** eingeplant werden: Die Engines werden als Home-Assistant-Apps (früher Add-ons) ausgeliefert, was Home Assistant OS oder Supervised voraussetzt — Home Assistant Core kann sie nicht installieren und benötigt stattdessen extern gehostete Wyoming-Dienste. Der dokumentierte Ausweg ist konkret statt theoretisch: openWakeWord wird als Docker-Container veröffentlicht, und die Wyoming-Integration wird dann auf dessen Adresse und Port (typischerweise 10400) gezeigt `[doc:user]` `[policy]`
...
- Für batterie- oder netzbeschränkte Satelliten **SOLLTE [SHOULD]** die **On-Device**-Erkennung (`microWakeWord`) bevorzugt werden: Nur Audio nach der Erkennung wird gesendet, sie funktioniert ohne Netzwerkverbindung und liefert genau drei vortrainierte Modelle — „okay nabu", „hey jarvis" und „alexa" — zum Preis kleinerer Modelle mit weniger Genauigkeitsspielraum. Warum diese Wahl überhaupt existiert, steht unumwunden da: openWakeWord „is too large to run on low-power devices like the S3-BOX-3". Zu beachten außerdem: Die Home-Assistant-Dokumentation bezeichnet S3-BOX und S3-BOX-Lite als „now discontinued" — das sollte [`ha/esp32-s3-box`](../esp32-s3-box/de.md) aufnehmen, wo sie alle drei Generationen als aktuell führt `[doc:user]`

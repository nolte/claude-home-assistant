...
- **SOLLTE NICHT [SHOULD NOT]** eine Sensor-Gruppe für numerische Aggregation einsetzen, die über die fixen `type`-Funktionen hinausgeht (z. B. gewichtete Mittel, gleitendes Fenster über Zeit, Ableitung) — dafür sind **min_max**, **statistics** oder **derivative** (`ha-automation/min-max`, `ha-automation/statistics`, `ha-automation/derivative`) gedacht; die Sensor-Gruppe bietet nur die wenigen dokumentierten Aggregate. Für ein in-Katalog liegendes numerisches Aggregat, das **nicht** zugleich eine Gruppen-Entität sein muss, ist `ha-automation/min-max` das fokussierte Werkzeug

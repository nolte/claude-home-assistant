...
**Naming-Bindung:** Dieses Plugin erbt die Namensform-Konvention aus `spec/claude/skill-agent-naming/` (claude-shared): Skills `<object-noun>-<action>`, Agents `<subject>-<role-noun>`, jeweils mit dem festen Domänen-Präfix `ha-`. **Geschlossene HA-Ausnahmeliste** (Familien-Suffix-Ausnahme gemäß §Inherited surfaces; ein Reviewer DARF sie NICHT flaggen; die Liste ist abschließend): die `*-solution`-Front-Door-Familie — `ha-solution`, `ha-integration-solution`, `ha-lovelace-solution`, `ha-automation-solution`, `ha-esphome-solution`, `ha-pixoo-solution` — deren bewusstes, uniformes Suffix den Orchestrierungs-Einstieg der jeweiligen Domäne benennt. Die Liste wächst ausschließlich mit einem neuen Domänen-Front-Door und wird im selben Change erweitert, in dem der Front Door entsteht; ein `*-solution`-Name, der hier fehlt, ist ein Finding. Alle übrigen Artefakte folgen der Konvention ohne Ausnahme (Naming-Uniformitäts-Pass 2026-07: sechs Altnamen zu `-map`/`-add`/`-provisioner`/`-deployer`/`-verifier`/`-reviewer` umbenannt, Deprecation-Stubs bis zum übernächsten Minor).
...
| [`ha/esphome-project-structure`](ha/esphome-project-structure/en.md) | ESPHome Project Structure (Repository Layout and Reuse) | ESPHome-Projektstruktur (Repository-Layout und Wiederverwendung) | draft | unversioned |
| [`ha/esphome-ha-driven-content`](ha/esphome-ha-driven-content/en.md) | ESPHome: Home-Assistant-Driven Device Content | ESPHome: Home-Assistant-gesteuerte Geräteinhalte | draft | unversioned |
...
| [`ha/esp32-s3-box-display-design`](ha/esp32-s3-box-display-design/en.md) | ESPHome Device: ESP32-S3-BOX Display Design (Icons, Colour, Structure) | ESPHome-Gerät: ESP32-S3-BOX-Display-Design (Icons, Farbe, Struktur) | draft | unversioned |
...
ESPHome-Cluster (`ha/esphome-config-patterns`, `ha/esphome-project-structure`, `ha/esphome-ha-driven-content`, `ha/esp32-s3-box`, `ha/esp32-s3-box-display`, `ha/esp32-s3-box-display-design`, `ha/assist-pipeline`): Anders als der übrige `ha/`-Korpus stammt dieser Cluster nicht aus `kamerplanter-ha` oder der HA-Developer-Doku, sondern aus vier eigenen Quellen, die jede Spec über **Evidenz-Tiers** je Anforderung ausweist — der offiziellen ESPHome-Komponenten-Doku ([`esphome.io`](https://esphome.io)), Espressifs Board-Support-Package ([`espressif/esp-bsp`](https://github.com/espressif/esp-bsp)) für Hardware-Fakten, ESPHomes eigenen Board-Referenzkonfigurationen ([`esphome/wake-word-voice-assistants`](https://github.com/esphome/wake-word-voice-assistants)) als normative Bindung, und dem realen Portfolio-Fixture [`nolte/esphome-configs`](https://github.com/nolte/esphome-configs) für gelebte Praxis. Wo eine Aussage nur im ESPHome-Quellcode belegbar ist, trägt sie ein eigenes `[source]`-Tier samt Hinweis, dass undokumentierte API keine Kompatibilitätszusage trägt; wo die Doku schweigt, sagen die Specs das ausdrücklich, statt zu schlussfolgern. HA-seitige Aussagen in `ha/assist-pipeline` sind nach `[doc:user]` / `[doc:dev]` getrennt. `ha/esp32-s3-box-display-design` erweitert diese Tier-Menge um drei Fremdquellen — `[ha-tokens]` (die Design-Tokens des HA-Frontends aus `home-assistant/frontend`, `src/resources/theme/`), `[md-icons]` (Googles System-Icon-Guidelines, denen MDI folgt) und `[wcag]` (W3C-WCAG-2.2-Erfolgskriterien) — sowie um `[derived]` für hier nachgerechnete Werte (Panel-Geometrie, RGB565-Quantisierung, Kontrastverhältnisse), die reproduzierbar sind statt zitiert.

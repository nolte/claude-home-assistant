...
- **MUSS [MUST]** den `translation_key` in `strings.json` unter `issues:` mit `title` und `description` hinterlegen, alle `translation_placeholders` im Text auflösen (Placeholder-Werte müssen Strings sein) und **keine** hartkodierten User-Strings im Python-Code lassen (Detail-Übersetzung folgt `ha/translations`)
- **MUSS [MUST]** den tatsächlichen `async_delete_issue(hass, domain, issue_id)`-Aufruf an der Auflösungsstelle platzieren (nicht bloß einen Pfad benennen), sodass das Issue verschwindet, sobald der zugrunde liegende Zustand behoben ist — eine fehlende Aufrufstelle lässt einen veralteten Repair stehen

...
- **MUSS NICHT [MUST NOT]** `min_max` als **Zeit-Statistik eines einzelnen Sensors** (Mittel/Min/Max über die Vergangenheit) zweckentfremden — dafür ist `statistics` (`ha-automation/statistics`) zuständig, weil `min_max` mehrere Quellen **zu einem Zeitpunkt** kombiniert und keinen Zeitverlauf einer einzelnen Quelle bildet
...
- **MUSS NICHT [MUST NOT]** `min_max` für **nicht-numerische Gruppen-Zustandsaggregation** (z. B. „eine von mehreren Türen offen", „mindestens ein Licht an") einsetzen — dafür ist eine **Gruppe** (`ha-automation/group`) das richtige Werkzeug, weil `min_max` ausschließlich auf numerischen Werten mit gemeinsamer Einheit rechnet

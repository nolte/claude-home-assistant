---
name: ha-dev-instance-provisioner
description: >-
  ...
  preparing `/config/custom_components` — so that `ha-integration-deployer`
  ...
  (that is `ha-integration-deployer`), to restart HA for a code refresh (that
...
- copy or modify integration code — that is `ha-integration-deployer`
...
Without this, the first `kubectl cp` of `ha-integration-deployer` fails on the missing parent directory.
...
- **Next:** deploy the integration with the `ha-integration-deployer` agent
```

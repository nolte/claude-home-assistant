...
- **MUST** declare `esphome: {min_version: ...}` at **no lower than** the floor that generation's own reference config pins — as of 2026-08 that is `2026.4.0` for the BOX-3 and `2025.5.0` for BOX and BOX-Lite, so there is no single family-wide floor to copy. It is a lower bound, not a target: a config that uses a feature introduced later carries that feature's floor instead, and the two must be reconciled deliberately rather than by leaving the config pinned below the syntax it emits (the `image:` platform form, new in `2026.7.0`, is the worked example — see [`ha/esp32-s3-box-display`](../esp32-s3-box-display/en.md)). Treat a bump as a reviewed change — ESPHome's audio components have changed schema repeatedly, so an unpinned config breaks silently on upgrade `[ref-config]` `[policy]`

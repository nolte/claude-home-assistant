...
- **MUST NOT** use `history_stats` to query or react to the **current real-time state** of an entity — a direct `state`/`numeric_state` trigger or a **template/threshold sensor** (`ha-automation/template`, threshold) is the right tool, because `history_stats` is a backward-looking aggregation over a window and does not yield the instantaneous value

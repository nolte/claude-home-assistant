...
- Graphic design guidance (colour theory, iconography, typography aesthetics) — this spec governs the mechanics of placement, not the taste applied to it; the design system layered on top of these mechanics is [`ha/esp32-s3-box-display-design`](../esp32-s3-box-display-design/en.md)
...
- **SHOULD** verify a new panel or geometry with `show_test_card: true` before debugging a layout, so a wrong `color_order` or offset is ruled out first — but **MUST** note that this conflicts with the refresh model below: the documentation states the test card is not drawn when `update_interval: never` is set, which this spec otherwise mandates. Showing the card therefore means temporarily restoring a real `update_interval`, and restoring `never` afterwards `[doc]`
...
- **MUST** pass a **background colour** whenever the font is anti-aliased (`bpp` greater than 1), and **MUST** put it in the position that overload expects — the two differ, which is the trap:
  ...
  - `it.printf(x, y, font, foreground, background, align, format, …)` — background **fifth**, before the align, because the variadic format arguments must come last. The documentation calls this "the full version of `printf`" and shows it for exactly this purpose; it was previously recorded here as undocumented, which is no longer true.

  Passing a background in the `print` position to `printf` does not fail loudly — it binds a different overload and the colour is consumed as a format argument `[doc]`
...
- **MAY** use `transparency: chroma_key` or `alpha_channel` to compose an image over a background instead of pre-baking the background into every asset. The documentation states that the display lambda's image functions "will draw or not draw the pixel, no blending with the background will be done", and that alpha blending is "useful mainly when using LVGL" — but that holds only for `RGB565` and `RGB`, whose draw paths take a binary decision on `alpha >= 0x80`. For **`GRAYSCALE` it is wrong**: that path computes `on = gray/255` and blends `color_on` against `color_off` per pixel, so a `GRAYSCALE` + `alpha_channel` image *is* alpha-blended in immediate mode, at one byte per pixel. That behaviour is the foundation of the icon system in [`ha/esp32-s3-box-display-design`](../esp32-s3-box-display-design/en.md) and is established from `esphome/components/image/image.cpp`, not from the documentation `[doc]` `[src]`
...
- **SHOULD** leave `draw_rounding` at its default of 2 unless artifacts appear; it rounds draw areas to a boundary for controllers that require it, and the `mipi_spi` documentation states plainly that the value **"Must be a power of 2"** — so a raised value is chosen from 4, 8, 16, not freely `[doc]`

...
- **Primärbeleg [HACS-Source]:** Im `zip_release`-Pfad ruft HACS `zipfile.ZipFile(asset).extractall(custom_components/<domain>/)` auf (`hacs/integration`, `repositories/base.py`) — das benannte Asset wird **direkt** dorthin entpackt; **`content_in_root` wird in diesem Pfad nicht ausgewertet** (es greift nur im Nicht-ZIP-/Single-File-Pfad). Bestätigt durch die Produktions-`hacs.json` von `alandtse/alexa_media_player` (`zip_release: true` + `filename` + `content_in_root: false`)
...
- **ZIP-Innenlayout** (geklärt 2026-06-25, nun primärbelegt): Das `zip_release`-Asset enthält den **Inhalt** von `custom_components/<domain>/` auf ZIP-Wurzelebene (das etablierte `integration_blueprint`-Muster); HACS entpackt es per `extractall` direkt nach `custom_components/<domain>/` und wertet `content_in_root` dort nicht aus. **Primärbeleg:** HACS-Source `hacs/integration` `repositories/base.py` (`zip_release`-Pfad, `zip_file.extractall(self.content.path.local)`) plus Produktionspräzedenz `alandtse/alexa_media_player`. Eine reale Test-Installation vor einer Default-Store-Aufnahme bleibt empfohlen, ist für den Mechanismus aber nicht mehr beweisbedürftig.

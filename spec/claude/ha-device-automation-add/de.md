...
- **MUSS [MUST]** für Conditions `async_get_conditions(hass, device_id)` implementieren; `CONDITION_SCHEMA` leitet von `DEVICE_CONDITION_BASE_SCHEMA` ab (Modulkonstante); `async_condition_from_config(config, config_validation)` ist `@callback`, liefert eine `bool`-Checker-Funktion und respektiert `config_validation`

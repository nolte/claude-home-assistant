...
description: "Augments an existing Home Assistant frontend module with one custom Lovelace strategy — a dashboard strategy or a view strategy — conforming to spec/ha/lovelace-strategies. Creates the strategy class with the static async generate returning the kind-correct shape (a dashboard strategy returns a views array; a view strategy returns a cards array, never views), the ll-strategy custom-element registrations, the dashboard-resource loading, the strategy.type custom:<id> reference, hass.callWS registry access with Promise.all, optional getConfigElement/getCreateSuggestions, and — for dashboard strategies — the window.customStrategies push for the community-dashboard dialog. Activate on \"add a dashboard strategy\", \"auto-generate views with a strategy\", \"create a custom Lovelace strategy\", or equivalent German requests. Do not activate for a static card (ha-lovelace-card-scaffold), custom view-layout elements or panels (ha/lovelace-views-panels), badges (ha-badge-add), or deploying to a live HA instance."
